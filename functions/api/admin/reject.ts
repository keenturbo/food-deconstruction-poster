...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    await rateLimitOrThrow(env, `rl:admin:reject:${ip}`);
    ...
    if (order.status === "rejected") {
      return json({ success: true, message: "订单已拒绝", orderId: order.id, status: order.status });
    }

    if (order.status !== "pending") {
      return json({ success: false, error: `订单状态不允许拒绝: ${order.status}` }, { status: 400 });
    }
    ...
    return json({ success: true, orderId: order.id, status: order.status, rejectedAt: order.rejectedAt });
  } catch (err: any) {
